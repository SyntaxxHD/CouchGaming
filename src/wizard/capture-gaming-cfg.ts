...
export async function captureGamingCfg(): Promise<void> {
  console.log('')
  console.log('Next: capture the TV-only layout.')
  console.log("1. I'll open Windows display settings.")
  ...
  if (open) {
    try {
      await run('cmd.exe', ['/c', 'start', 'ms-settings:display'], { allowNonZero: true, timeoutMs: 5000 })
    } catch {
  ...
  console.log(`Gaming layout captured to ${paths.gamingCfg}`)
